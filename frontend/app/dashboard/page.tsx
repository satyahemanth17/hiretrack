...
export default function DashboardPage() {
  ...
  const activeTab = { borderBottom: "2px solid var(--primary)", color: "var(--primary)", fontWeight: "500" as const };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--surface)" }}>
      <nav style={navStyle}>
        ...
        <a href="/matcher" className="text-sm" style={{ color: "var(--text-secondary)" }}>Matcher</a>
      ...
      <div style={{ padding: "24px" }}>
        <div className="flex gap-6 mb-6 border-b" style={{ borderColor: "var(--border)" }}>
          <button onClick={() => setTab("board")} className="pb-3 text-sm" style={tab === "board" ? activeTab : inactiveTab}>Board</button>
          <button onClick={() => setTab("analytics")} className="pb-3 text-sm" style={tab === "analytics" ? activeTab : inactiveTab}>Analytics</button>
        </div>
